...
hook แบบ inject = "เตือน" (model ยังเลือกไม่ทำตามได้) ที่ "บังคับ" ได้จริงคือ `PreToolUse` deny + **verify ที่ hook รันเอง** (ปลอมไม่ได้เพราะ hook รันเทสต์เอง ไม่เชื่อคำอ้าง) การันตีสูงสุด "ห้าม merge ถ้าไม่เขียว" อยู่ที่ **CI required-check + branch ruleset**: `t4-project-bootstrap` ให้ทั้ง workflow (`lint` · `typecheck` · `test` · `build` แยกเป็น required check คนละตัว, ชุด e2e แยกต่างหาก และ CD ที่ gate ด้วย verify เขียว) และคำสั่งตั้ง ruleset ที่ทำให้เป็น required + ห้าม push ตรงเข้า `main` — ดู [`ci-cd-layer.md`](./skills/t4/t4-project-bootstrap/references/ci-cd-layer.md) ชั้นนี้คุมการ merge บนเว็บของคนได้ด้วย — local hook คุมได้แค่คำสั่งที่ agent รัน ส่วน repo ที่ผ่าน bootstrap พก hook ชุดเดียวกันไปเองผ่าน git (ไม่ต้องมี plugin)
