...
#     test-non-t4-route-rule (gives an existing case a ROUTE. The 'use the general
#       skills directly' sentence is kept and the suite asserts it is still there,
#       so nothing was withdrawn.)
#     test-number-unit-rule (adds a UNIT check beside the register rules, and gives
#       the delegation-side statement a second home. clink-subagents keeps its own
#       wording and the suite asserts it is still there, so nothing was withdrawn.)
#     test-evidence-scope-rule (extends an existing rule to ACTION verdicts. The rule
#       it extends is kept and the suite asserts the extension is framed as the rule
#       being FOLLOWED, so nothing was withdrawn.)
#     test-scope-check-rule (adds a SECOND check to step 1 beside the leading-prompt
#       audit. Step 1's own wording is kept and the suite asserts it is still there,
#       so nothing was withdrawn -- the issue is explicit that precision was never
#       the thing missing.)
...
#     test-blinded-agentic-rule (names a THIRD state beside chat and agentic. The
#       chat-vs-agentic rule is kept and the suite asserts it is still there, so
#       nothing was withdrawn; a `hasnt` here would have to invent the wording it
#       excludes, which is the #113 defect this file exists to avoid.)
...
test-bro-rule test-repo-self-bootstrap test-clink-prefix-rule test-prompt-audit-rule test-surgical-completion-rule test-blinded-agentic-rule test-synthesis-steps-rule test-tracker-body-edit-rule test-restore-mechanism-rule test-bro-trigger-rule test-scope-check-rule test-evidence-scope-rule test-number-unit-rule test-non-t4-route-rule"
cd "$REPO_ROOT" || exit 0
