...
- **[using-clink](./skills/multi-agent/using-clink/SKILL.md)** — the family entry: which of the four below you actually want, decided **before** you spend a call. Work versus judgment is the whole routing decision and it is the one that gets made backwards — a panel convened for a question one worker could answer burns several lanes to produce an opinion where a fact was wanted; a worker sent to produce judgment returns a confident answer nobody checked. `clink-masteragent` is named there as a precondition, not a fourth option.
...
- **[clink-masteragent](./skills/multi-agent/clink-masteragent/SKILL.md)** — what a master agent owns and can never delegate (decomposition, integration, final verification, trust boundaries, the delegation contract), and how to pick the model for a delegation from measured scores rather than memory. Carries the score table in the file itself, because an agent with nowhere to look it up chooses from recollection — and the session that produced this skill did exactly that, picking a model that was weaker on the axis the task needed. Read it **before** any `clink` call.

- **[clink-debug](./skills/multi-agent/clink-debug/SKILL.md)** — the single home for delegating a bug hunt: route a small bug to one cheap worker and escalate only what survives, define the evidence that travels with it, and never ask the lineage that proposed a hypothesis to falsify it.
...
- **[using-design](./skills/design/using-design/SKILL.md)** — The family coordinator; routes a design task to the right skill below.
...
- **[using-qwen38](./skills/qwen38/using-qwen38/SKILL.md)** — the router the model loads at session start: task shape → skill to load → gate to finish with, plus the four rules (brief table first, run and paste outputs, never install / search for tools / spawn agents, stop at the report). Injected into the model itself at SessionStart (startup / clear / compact) by `hooks/session-start` inside the skill's own folder, wired only from the Qwen profile settings files (`~/.claude-xeno-exl3.json`, `~/.claude-xeno.json`, `~/.claude-9arm.json`), so a frontier session never sees it.
- **[qwen38-skill-style](./skills/qwen38/qwen38-skill-style/SKILL.md)** — the authoring standard for this family: nine rules each with the run behind it, plus the one stated exception for the harness, the file shape, and the pre-PR checklist including one with/without pair.

- **[qwen38-think](./skills/qwen38/qwen38-think/SKILL.md)** — the thinking the model skips, as steps it must write down: an assumptions table and a three-flaw check (contradiction, missing fact, impossible ask) before the first edit, each flaw checked by a command; when stuck, one observation, one hypothesis, one falsifying command. Pushback is a written line, never a question that stops the work. Measured 3/4 on all four planted briefs without it and 4/4 with it — unaided the model sees the flaw and then asks a developer who is not there, or ships a knowingly wrong answer as done.

- **[qwen38-code-gate](./skills/qwen38/qwen38-code-gate/SKILL.md)** — the done-gate for a code change: the brief table for code (the test that goes red then green, the error path, the doc line, no new dependency), then six commands with pass conditions — diff scope, placeholders, secrets, the repo's tests with the last line pasted, lint or typecheck if configured, and the RED line before the GREEN one. Measured honestly: the code itself was at the fixtures' ceiling in every arm, and the gate's one effect is test discipline, RED-before-GREEN in 2 of 6 gated runs against 0 of 3 ungated.

- **[qwen38-claude-code](./skills/qwen38/qwen38-claude-code/SKILL.md)** — the harness guide, and the family's one full-coverage exception. Claude Code sends 31 tools and 87,799 characters of their schema on every request against 6,186 for the whole system prompt, and a schema says what a tool accepts, never what it is for — so the model touched 12 of the 31 across 44 recorded runs. Not a decision, a gap. This gives a verdict for every one of them (use / use when X / do not, each with its reason), which tool for which want, what to do when a tool returns an error including a harness denial, and how a session actually works. `harness-tools.txt` records the names from a captured request so a tool a future version adds fails the suite instead of going uncovered.
