...
| `mcp__pal__clink(cli_name="codex")` | OpenAI Codex — **`gpt-5.6-sol` and `gpt-5.6-luna`; skip `terra` and `gpt-5.5`, both dominated** (ladder + standing effort cap in `clink-subagents`) | **Code-centric** — syntax correctness, implementation details, edge cases | subprocess, `--json` | ~10-15s for a trivial prompt, but **400-530s measured for a read-heavy round** against a real repo (2026-07-31) — budget minutes, not seconds |
...
|---|---|---|
| `codex` | ✅ `-m` — **`gpt-5.6-luna` for routine rounds, `gpt-5.6-sol` when a round needs above Luna's ceiling of 51**; `gpt-5.6-terra` and `gpt-5.5` are dominated, skip them (validated; invalid → 400) | ✅ `low\|medium\|high\|xhigh\|max` — **capped: `high` on `sol`, `xhigh` on `luna`, `max` never** (ladder + rationale in `clink-subagents`) |
...
- **Escalate effort on the round that matters, not every call — and escalate the *model*, not past the effort cap.** Run a cheap first round (`gpt-5.6-luna` at `high`) to surface positions; then for the **adversarial round** or the final consequential call, move to `gpt-5.6-sol` at `high`. **`max` is not an available escalation** — the last rung buys +1 index point for a near-doubled quota burn, which is the mistake this line used to recommend. See the ladder in `clink-subagents`.
- **Widen the *model* spread, not just the CLI spread.** Real cognitive diversity comes from *different backend families*, not the same model three times. A strong cheap round: `codex` → `gpt-5.6-luna` at `high`, `antigravity` → `Gemini 3.1 Pro (High)`, `cursor` → `cursor-grok-4.5-high`, gateway model via `claude-9arm` — four genuinely different lineages in one fan-out. Pick the client by **whose quota it spends**, not merely by whether it carries the model — see *Quota routing* below.
- **Steep diminishing returns on effort, with numbers.** On `gpt-5.6-sol` the ladder is +4, +2.5, +2, **+1** while quota burn triples from `medium` to `max` ($0.31 → $1.04 of AA cost-proxy). `medium`/`high` is the value sweet spot; **`max` is capped out entirely** and `xhigh` belongs on `luna`, where it costs $0.10 and matches Sol at `low`. Blanket-`max` on a 3-agent × 3-round loop is a multi-minute, quota-heavy operation for almost no marginal signal — full ladder in `clink-subagents`.
