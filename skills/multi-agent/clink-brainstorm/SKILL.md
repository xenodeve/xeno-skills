...
| `mcp__pal__clink(cli_name="codex")` | OpenAI Codex — **`gpt-5.6-sol` only here**, at `medium`/`high` | **Code-centric** — syntax correctness, implementation details, edge cases | subprocess, `--json` | ~10-15s for a trivial prompt, but **400-530s measured for a read-heavy round** against a real repo (2026-07-31) — budget minutes, not seconds |
...
|---|---|---|
| `codex` | ✅ `-m` — **`gpt-5.6-sol` for every brainstorm round**; `gpt-5.6-terra` and `gpt-5.5` are dominated and `gpt-5.6-luna` is too weak to vote here (see below) (validated; invalid → 400) | ✅ `low\|medium\|high\|xhigh\|max` — **use `medium`, escalate to `high`; `xhigh` and `max` are capped out** |
...
- **Codex here is `gpt-5.6-sol` at `medium`, escalating to `high` for the round that matters. Nothing below that, and nothing above it.** `medium` (53.5) to surface positions; `high` (56) for the adversarial round or the final consequential call. **`max` is not an available escalation** — the last rung buys +1 index point for a near-doubled quota burn, which is the mistake this line used to recommend.

  **Why the cheap small model belongs in `clink-subagents` and not here.** There, a weak answer is recoverable — the leaf is verifiable, you run the test and catch it. **A brainstorm answer is the judgment itself; there is no test to catch it with.** And it does not merely cost you one weak vote: it poisons the synthesis, because you cannot distinguish *"this agent dissents because it sees something the others missed"* from *"this agent dissents because it is the weakest model in the room."* `gpt-5.6-luna` tops out at 51 and sits at 46 on the routine rung — below Claude Opus 4.6 (53) and under every other lane you would fan out to — so its disagreement carries no information. The volume argument does not rescue it either: a round is three or four calls, so `sol`/`medium` over `luna`/`high` costs well under a dollar of quota-proxy per round while buying the difference between a competitive voice and the weakest one. Full ladder and the delegation-side rules in `clink-subagents`.
