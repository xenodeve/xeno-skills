...
- **Codex here is `gpt-5.6-sol` at `medium`, escalating to `high` for the round that matters. Nothing below, nothing above.** `medium` (53.5) to surface positions; `high` (56) for the adversarial round or the final consequential call.
  ...
  **Why the range stops at `high`.** Marginal index per unit of quota collapses past it. On Sol: `low`→`medium` buys **+4 for $0.11**, `medium`→`high` buys **+2.5 for $0.14** — then `high`→`xhigh` buys +2 for $0.23, and `xhigh`→`max` buys **+1 for $0.36**, a sixth the value of the first step. A 3-agent × 3-round loop at the top of the ladder ends a working session in a handful of calls and buys almost nothing for it. Full ladder in `clink-subagents`.
