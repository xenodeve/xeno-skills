...
- **Cursor's tools die headlessly unless the preset carries `--auto-review`, and the agent misdiagnoses it.** Once `approvalMode` in `~/.cursor/cli-config.json` is `allowlist` — which happens as soon as you approve one command interactively — a headless run has no way to decide on a tool call that isn't already listed, because it cannot prompt a human. `Read`, `Shell`, `Grep`, `Glob` and MCP access all fail, and **the agent answers from the prompt text alone with exit 0**, so the call looks successful while the client is effectively non-agentic. `--auto-review` hands the decision to a server-side classifier and restores the safe calls; `--force`/`--yolo` also work but permit everything. **Do not trust the agent's own explanation here:** in the broken state it blames a *"PreToolUse hook"* and quotes a bash syntax error, repeatedly and convincingly, when no hooks file exists anywhere on the machine. Check `approvalMode` yourself. A subagent reporting a root cause is a hypothesis like any other return value — the rule below applies to its diagnoses, not only its code.
