...
7. **Hand it the baseline skills — on every delegated coding call.** `tdd` and `karpathy-guidelines` are procedural: they apply cleanly even to a small model, and they are what keep a worker on structure and honest about gaps where an unguided one drifts and fabricates. Add `simplify` and `post-mortem` when the task is a cleanup or a bug write-up. **[`clink-brainstorm`](../clink-brainstorm/SKILL.md) is not covered by this** — a panel produces judgment, and `tdd` has nothing to act on there.
