...
| `t4-gate` | `PreToolUse` (`Bash`) | **Blocks** `gh pr create` with no referenced issue; blocks dangerous git (`reset --hard`, force-push, `clean -f`, `branch -D`); before `gh pr merge` runs the repo's `verify` itself — and, with `"requireGreenCI"`, checks the PR's CI too | **Enforces** the discrete, checkable rules (hard) |
...
2. **Server-side gate (the real guarantee).** Install the workflows from `references/ci/` and make `lint`/`typecheck`/`test`/`build` **required status checks** on `main`, with **direct pushes to `main` disallowed** — the full procedure, ruleset commands, and CD gating are in **`ci-cd-layer.md`**. This is the only layer that also catches a human merging on the web and that `--no-hooks` can't skip. Keep its fast subset in sync with `.claude/t4.json` `"verify"`.
3. **CI check from the local side (opt-in fallback).** Where a ruleset isn't available, `.claude/t4.json` `"requireGreenCI": true` makes `t4-gate` run `gh pr checks` before `gh pr merge` and **deny while any check is failing or still pending** — including under `"autoMerge"`/`"afk"`. Weaker than a ruleset (it binds only agent-run commands), so use it as a stopgap and say so.
...
| **The gate denies a legitimate command** | Detection is anchored to a **command position** and the git/`gh` **subcommand**, so the pattern inside a quoted message (`git commit -m "reset --hard"`) doesn't trip it — but a genuine invocation reached another way (a script, an alias) can still be caught → run it yourself, or rephrase. |
...
| **`gh pr merge` blocked with "checks are not green"** | `"requireGreenCI": true` and a check is failing **or still pending** (`gh pr checks` exits 8 while pending) → wait for CI, or fix the red check. In a repo with no CI at all the same call reports non-zero → drop the flag. See `ci-cd-layer.md`. |
