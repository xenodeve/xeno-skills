...
|-----|----------|-------------|---------|
| `CLAUDE.md` | root | bootstrap | Agent operating manual: north-star, repo structure, commands, memory + notify wiring, `docs/agents/*` pointers, architecture map, and the **session-end step** — report each rule that did not hold as a `skill-feedback` issue on the skill library (`t4-agent-memory`). |
