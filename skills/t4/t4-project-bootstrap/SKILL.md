...
6. **Install the hooks layer** from `references/hooks-layer.md` — copy the marker (`.claude/t4.json`), the `.claude/hooks/` scripts + `run-hook.cmd`, merge the hook entries into `.claude/settings.json`, and write `using-t4.snapshot.md`. This keeps a session on the rails: session-start injects `using-t4`, a per-turn reminder re-anchors it, and a `PreToolUse` gate blocks a PR with no issue and dangerous git. **Arm the ship gate** by setting `.claude/t4.json` `"verify"` to the repo's test command, and install `references/ci/t4-verify.yml` as `.github/workflows/t4-verify.yml` + make it a required check on `main` (the server-side gate that also covers human merges). Tell the user what the gate will block.
7. **Install the guards layer** from `references/guards-layer.md` — copy `references/guards/` into `<repo>/.githooks/`, tell the user to run `git config core.hooksPath .githooks`, and wire the same two scripts into the CI gate. This is the tier that binds Codex/Gemini/humans; the Claude hooks in step 6 do not.
...
- **`references/hooks-layer.md`** + **`references/hooks/`** + **`references/ci/`** — the workflow-hooks layer (path A): the `.claude/t4.json` marker, the `.claude/hooks/` scripts + `run-hook.cmd`, the `settings.json` hook entries, and the `t4-verify.yml` CI ship-gate. Session-start / prompt-reminder / PreToolUse-gate keep a session on the rails; the `"verify"` command + CI required-check are the un-forgeable ship gate. The scripts are byte-identical to the `xeno-skills` plugin's `hooks/` (a repo test enforces the sync).
