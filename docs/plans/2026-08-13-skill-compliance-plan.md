...
- **`clink` is readable at the boundary, which covers more than it first appears.** 35 of this session's 40 delegations went to a foreign CLI, so what the worker did in private is not recorded — but the rule that matters most is a fact about the master's own prompt. `clink-subagents:140` requires handing the worker `karpathy-guidelines` on every call and `tdd` whenever it writes code, and `clink-brainstorm:243` makes pasting the skill's content into the prompt the reliable method. **That handoff sits inside the `tool_use` input**, so *which skills the worker was given* — and, by hashing the pasted text against the shipped file, *which version* — is a string match on a harness-written record. Deterministic, no model. The same holds for the sentinel requirement and for the fresh `continuation_id` that `clink-debug`'s provenance rule demands of a falsify seat, which is a parameter on the call.
- **What is left after that** is whether the worker followed what it was handed, which moves to the undecidable pile and is counted. **It is the same ceiling the master already has** — #130 is a skill invoked, in context, and the step still not run — so it is the honest limit of the approach appearing one level down, not a defect specific to clink.

**And the clink boundary is ours to move.** `xenodeve/pal-mcp-server` is developed alongside this repository, so the foreign-format problem is a fact about today's implementation rather than a constraint: a fork that logs each call in our own format, or that takes a `skills` parameter and records what it attached, turns the boundary into a first-class record. **That work belongs in that repository's tracker and is deliberately not folded in here** — but the wall is not given, and this plan should not have implied it was.
