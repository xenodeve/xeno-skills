...
| **Q5 evaluator in the harness** | **[L]** yes — `type: "prompt"`, and it is **Haiku 4.5**, named in `modelUsage` | **[L]** no — a `prompt` handler beside a `command` control produced `skipping prompt hook in …: prompt hooks are not supported yet`, and the control still fired | **[L]** yes — **[B]** queued only if a `promptHookClient` exists | **[L]** no — `prompt hooks are not currently supported`, its own log |
| **Q6 tool-using hook** | **[L]** yes — `type: "agent"` read a file off disk and blocked with its contents | **[L]** no — same shape, `skipping agent hook in …: agent hooks are not supported yet`, sibling unaffected | **[L]** no — and the entry **voids the whole file**, silently | **[L]** no — `unsupported hook type: "agent"`, file named |
