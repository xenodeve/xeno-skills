...
| Item | Status | Gate | Next action |
|---|---|---|---|
| #354 `qwen38/` family — `using-qwen38` router + `qwen38-skill-style` authoring standard | 🟢 | PR open from `feat/354-qwen38-family` | Built 2026-09-05, extended 2026-09-06 with `qwen38-claude-code` and the SessionStart hook. Pairs for #356 and #357 are recorded on their issues; the family's own daily-driver replay is still open under #355. |
...
| #353 `design-ship-gate` — an executable done-gate for a small model | ✅ | — | **Merged to `main` 2026-09-06 (PR #360).** 8/8 ×3 with the gate against 6/8 ×3 without; the design suite alone scored 4/8 ×3. Review findings that did not block it are #359. |
