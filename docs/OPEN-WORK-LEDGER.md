...
| Item | Status | Gate | Next action |
|---|---|---|---|
| #354 `qwen38/` family — `using-qwen38` router + `qwen38-skill-style` authoring standard | 🟢 | PR from `feat/354-qwen38-family` | **Opened 2026-09-05 12:35 on the developer's go.** Built in parallel with the Qwen-3.8-27B-Tuning skill-set A/B (4 arms × 3 reps, running) and NOT used by it; that experiment's data feeds the standard, then the family is tested against the model. Evidence base so far: #353's A/B (8/8 with the gate vs 6/8 without), the two literal-compliance traps (`gh` hunt, Thai spellchecker install), #134. **2026-09-06: in daily use on the developer's go** — `using-qwen38/hooks/session-start` injects the router at SessionStart from the three Qwen profile settings files (never the global settings); live check: a fresh `claude-xeno-exl3 -p` session named the map and the `THINK:` line unprompted. `tests/skills/test-qwen38-session-start.sh` (11). |
| #358 `qwen38-claude-code` — every harness function, not only measured gaps | 🟢 | a rep with the router present, to see whether the catalogue changes which tools get reached for | **Goal, the developer 2026-09-06: make Qwen use Claude Code as well as it possibly can, through the skill's guidance.** The family's one exception to measured-gaps-only: Claude Code sends 31 tools and 87,799 chars of schema per request; Qwen touched 12 across 44 streams because a schema says what a tool accepts and never what it is for. All 31 now carry a verdict; `harness-tools.txt` is recorded from a real request and the test derives coverage from it; 16 KB cap, reason in `qwen38-skill-style`. |
| #353 `design-ship-gate` — an executable done-gate for a small model | ✅ | — | **Built 2026-09-05, uncommitted.** Nine Qwen3.8-27B pages from one brief broke rules `design-rules` states in words (fonts 5/9, OG 6/9, collisions 3/9) — #134's finding again — so the skill is eight commands with pass conditions plus three Playwright scripts, run before "done". Dirty-run against the audited pages: check 4 found the 4 invisible stat numbers, check 5 the 56 px overflow, check 6 the 01–07 ruler over the `h1`, controls all 0; one audited collision (a glass lens over a tagline) is a stated known miss. 43 assertions in `test-design-ship-gate.sh`. Not yet used by the model it was written for — the first Qwen run through it is the evidence to close. |
| #355 epic — `qwen38/` for a full-stack developer's day: code / think / api / workflow / debug / frontend gates | 🟡 | each slice needs its harness before it is buildable | **Opened 2026-09-05 13:10.** The developer's goal: move daily work from frontier models to Qwen3.8-27B. Seven slices, each = skill + task brief + scoring harness + one with/without pair, or it does not ship. |
| #356 `qwen38-code-gate` (slice 1 of #355) | ✅ | — | **Closed 2026-09-05 19:30 with the pair.** Code at the fixtures' ceiling in every arm; the gate's measurable effect is RED-before-GREEN 2/6 vs 0/3. Kept as the family's process skill; real value to be shown on the daily-driver replay. |
| #357 `qwen38-think` (slice 2 of #355) | ✅ | — | **Closed 2026-09-05 19:30 with the pair.** No skill 3/4 on all four planted briefs (asks and stops, or ships a knowingly wrong answer as done); the skill alone 4/4 on all four; the family 4/4 once the report shape was unified (rule 9). |
