...
| Item | Status | Gate | Next action |
|---|---|---|---|
| #354 `qwen38/` family — five skills, the router's SessionStart hook, and the authoring standard | ✅ | — | **Merged to `main` 2026-09-06 (PR #361).** Pairs for #356 and #357 are on their issues; #358 added full harness coverage. What is still open is the daily-driver replay, under #355. |
| #358 `qwen38-claude-code` — every harness function, not only measured gaps | ✅ | — | **Merged to `main` 2026-09-06 (PR #361).** All 31 tools Claude Code sends carry a verdict; `harness-tools.txt` is recorded from a real request and the test derives coverage from it. **Unmeasured:** whether the catalogue changes which tools get reached for — the pair on #355 predates it. |
